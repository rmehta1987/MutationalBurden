...
class AffineTransform(transforms.AffineTransform, nn.Module):
...
class buildFlow(nn.Module):
    def __init__(self, input_sz, hidden_szs, latent_sz, num_transforms: int=1, num_blocks: int=1, dropout_probability: float=0.1, 
                 permute_flow: bool=False, use_batch_norm: bool=False, num_bins: int=2, device: bool=False, tail_bound: float=8.0, base=None):
        super().__init__()
        ...
        self.burden_critic_baseline_nsf_pyro(base=self.base)
        #self.burden_critic_baseline_affine_autoregressive_pyro(base=self.base)
        #self.burden_critic_baseline_spline_pyro(base=self.base)
        #self.burden_critic_baseline_affine_pyro(base=self.base)
    
    def burden_critic_baseline_nsf_pyro(self, base=None):
        dim = self.latent_sz
        split_dim = self.latent_sz // 2
        ...
        if base is not None:
            ...
            torch.distributions.Normal(
                torch.zeros(self.latent_sz),
                torch.ones(self.latent_sz),),
                1,)
        ...
        for i in range(self.num_transforms):
            self.flows.append(transform.with_cache())
            if self.permute and i < self.num_transforms - 1:
                permutation = torch.randperm(dim)
                self.flows.append(transforms.Permute(permutation))
            if self.batch_norm and i < self.num_transforms - 1:
                bn = transforms.BatchNorm(dim)
                self.flows.append(bn)
        self.transforms = nn.ModuleList(self.flows)
        
        self.a_dist = torch.distributions.TransformedDistribution(base_dist, self.flows)
    
    def burden_critic_baseline_spline_pyro(self, base=None):
        ...
        self.flows = []
        self.flows.append(transforms.Spline(self.latent_sz, count_bins=16, bound=3.))
        self.transforms = nn.ModuleList(self.flows)
        self.a_dist = torch.distributions.TransformedDistribution(base_dist, self.flows)
    
    def burden_critic_baseline_affine_pyro(self, base=None):
        ...
        self.flows = []
        transform = AffineTransform(torch.zeros(self.latent_sz),torch.ones(self.latent_sz))
        self.flows.append(transform)
        transform = AffineTransform(torch.zeros(self.latent_sz),torch.ones(self.latent_sz))
        self.flows.append(transform)
        #self.transforms = nn.ModuleList(self.flows)
        #self.a_dist = torch.distributions.TransformedDistribution(base_dist, self.flows)
        self.a_dist = TransformedDistribution(base_dist, self.flows)

    
    def burden_critic_baseline_affine_autoregressive_pyro(self, base=None):
        dim = self.latent_sz
        hidden_dims = [3 * self.latent_sz + 5, 3 * self.latent_sz + 5]
        nonlinearity = nn.SiLU()
        ...
        if base is not None:
            ...
            torch.distributions.Normal(
                ...
                torch.ones(self.latent_sz),),
                1,)
        arn = AutoRegressiveNN(self.latent_sz, hidden_dims, nonlinearity=nonlinearity, skip_connections=skip_connections)
        if self.device:
            arn.to('cuda')
        transform = transforms.AffineAutoregressive(arn, log_scale_min_clip=-3.0)
    ...
    def log_prob(self, value):
        ...
        """
        #if self.a_dist._validate_args:
        #    self.a_dist._validate_sample(value)
        event_dim = len(self.a_dist.event_shape)
        log_prob = 0.0
        ...
        base_log_prob = self.a_dist.base_dist.base_dist.log_prob(y)
                                             
        return base_log_prob, log_prob
    
    #def log_prob_simple_affine(self, value):


    def rsample(self, sample_shape=torch.Size()):
        """
        Generates a sample_shape shaped reparameterized sample or sample_shape
        shaped batch of reparameterized samples 
        """
        if sample_shape is None:
            return self.a_dist.rsample()
        else:
            return self.a_dist.rsample(sample_shape)
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        if sample_shape is None:
            ...
            return self.a_dist.sample(sample_shape)
...
class BurdenEncoder(nn.Module):
    def __init__(self, input_sz, hidden_szs, latent_sz, num_layers):
        ...
        self.log_var_min_clip = -5.0
        self.log_var_max_clip = 3.0
        ...
        #self.global_spike = nn.Linear(self.hidden_szs, 1) 
    ...
    def forward(self, x):
        ...
        log_var = self.clamp_preserve_gradients(log_var, self.log_var_min_clip, self.log_var_max_clip)
    ...
    def clamp_preserve_gradients(self, x, min, max):
    # This helper function clamps gradients but still passes through the gradient in clamped regions
        return x + (x.clamp(min, max) - x).detach()


class BurdenIWEncoder(nn.Module):
    def __init__(self, input_sz, hidden_szs, latent_sz, num_samples, num_layers):
        super().__init__()
        self.input_sz = input_sz # Max number of snps 
        self.hidden_szs = hidden_szs 
        self.n_layer = num_layers
    ...
    def forward(self, x):
        ...
        # Create spike probablities
        spike = self.spike_linear(z) # create probabilities
        #spike = torch.nn.functional.relu(selectors)*-1.0 # logscale probabilities, log(pi) 
        spike = torch.nn.functional.logsigmoid(spike) # logscale probabilities, log(pi) 
        ...
        mean_iw = mean.unsqueeze(1).repeat(1, self.num_samples, 1)
        log_var_iw = log_var.unsqueeze(1).repeat(1, self.num_samples, 1)
        spike_iw = spike.unsqueeze(1).repeat(1, self.num_samples, 1)

        # reparamterize to sample posterior true beta
        post, post_dist = self.reparameterize(mean_iw, log_var_iw)
        return post, post_dist, mean, log_var, spike_iw
...
class BurdenDecoder(nn.Module):
    def __init__(self, input_sz, hidden_szs, latent_sz, log_var, num_layers):
        super().__init__()
        self.input_sz = input_sz # Max number of snps 
        self.hidden_szs = hidden_szs 
        self.n_layer = num_layers
...
class BurdenIWDecoder(nn.Module):
    def __init__(self, input_sz, hidden_szs, latent_sz, log_var, batch_shape, num_samples, num_layers):
        super().__init__()
        self.input_sz = input_sz # Max number of snps 
        self.hidden_szs = hidden_szs 
        self.n_layer = num_layers
    ...
    def forward(self, x):
        ...
        x_hat = self.decoder(x) # [batch, samples, variant_num]
        mean = self.mean_linear(x_hat)
        if self.log_var:
            log_var = torch.log(torch.nn.functional.softplus(self.log_var_linear(x_hat)))
            log_var = self.clamp_preserve_gradients(log_var, self.log_var_min_clip, self.log_var_max_clip)
            return mean.view(-1, self.num_samples, x.shape[-1]), log_var.view(-1, self.num_samples, x.shape[-1])
        else:
            return mean.view(-1, self.num_samples, x.shape[-1]) # [batch, num_samples, variant_num]
...
class VariationalBurden(nn.Module):
    def __init__(self, num_variants: int=10, hidden_sz: int=5, latent_sz: int=1, num_samples=1, batch_size: int=25, lambda_param: float=0.1,
                 ...
                 tail_bound: float=8.0, reinforcement_learning: bool=False, device: bool=False):
        super().__init__()
        ...
        self.temperature = torch.tensor(.15)
        self.batch_size = batch_size
        self.reinforcement_learning = reinforcement_learning
        self.noise_parameter = nn.Parameter(torch.tensor(1.))
        self.kl_weight_spike =1.0
        self.kl_weight_slab = 1.0
        self.num_samples = num_samples
        # Build pyro based priors
        #self.latent_prior, self.latent_net = self.burden_critic_baseline_nsf_pyro()
        self.latent_prior = buildFlow(self.input_sz, self.hidden_sz, self.latent_sz, num_transforms=self.num_transforms, num_blocks=self.num_blocks,
        ...
        # Build decoder
        self.decoder = BurdenIWDecoder(self.input_sz, self.hidden_sz, self.latent_sz, False, self.batch_size, self.num_samples, num_layers=2)
    ...
    def iwae_latent_variable_update_with_spike_and_decoder(self, x, mask, standard_error, posterior_beta, posterior_dist, logit_spike, selected, 
        ...
        # Find q(z|x)
        log_Q_e_Fx = posterior_dist.log_prob(posterior_beta)*mask
        ...
        p_z_log_prob = p_z.log_prob(posterior_beta)*mask
        ...
        log_Q_e_Fpi = q_sampler.log_prob(pi_samples)*mask
        ...
        p_sampler = torch.distributions.relaxed_bernoulli.RelaxedBernoulli(temperature=self.temperature, probs=0.2)
        log_P_d_Fpi = p_sampler.log_prob(pi_samples)*mask
        ...
        # Find p(x|z)
        posterior_beta = torch.mul(torch.clamp(pi_samples,1e-6),posterior_beta)
        mean_estimate, log_var_estimate = self.decoder(posterior_beta)
        beta_hat_sampler = torch.distributions.Normal(loc=mean_estimate*mask, scale=torch.mul(self.noise_parameter+(standard_error),mask)+1e-6).log_prob(x)*mask
        #beta_hat_sampler = torch.distributions.Normal(loc=mean_estimate*mask, scale=torch.mul(log_var_estimate,mask)+1e-6).log_prob(x)*mask
        ...
        with torch.no_grad():
            eps = torch.randn_like(true_beta_var)
            true_beta =  true_beta_mean+eps*torch.exp(0.5*true_beta_var)
            beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate*mask, scale=torch.mul(self.noise_parameter+(standard_error),mask)+1e-6)*mask
            #beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate*mask, scale=torch.mul(log_var_estimate,mask)+1e-6)**mask
            #beta_hat_sampler_2 =  torch.distributions.Normal(loc=posterior_beta, scale=(standard_error+1e-6).pow(2))
            recon = beta_hat_sampler_2.sample()*mask
    ...
    def iwae_latent_variable_update_with_spike_and_decoder_no_mask(self, x, mask, standard_error, posterior_beta, posterior_dist, logit_spike, selected, 
                                                           true_beta_mean, true_beta_var, num_samples, batch_size):
        """Importance weighted elbo
        ...
        # find p(pi)
        p_sampler = torch.distributions.relaxed_bernoulli.RelaxedBernoulli(temperature=self.temperature, probs=0.3)
        log_P_d_Fpi = p_sampler.log_prob(pi_samples)
        ...
        posterior_beta = torch.mul(pi_samples,posterior_beta)
        #mean_estimate, log_var_estimate = self.decoder(posterior_beta)
        mean_estimate, _ = self.decoder(posterior_beta)
        #scale_estimate = torch.mul(log_var_estimate.mul(0.5).exp_(),mask)+1e-6
        mean_estimate = torch.mul(mean_estimate, mask)
        #beta_hat_sampler = torch.distributions.Normal(loc=mean_estimate, scale=scale_estimate).log_prob(x)
        beta_hat_sampler = torch.distributions.Normal(loc=mean_estimate, scale=torch.mul(self.noise_parameter+(standard_error),mask)+1e-6).log_prob(x)*mask
        beta_hat_loss = torch.sum(beta_hat_sampler, axis=-1,keepdim=True)
        
        with torch.no_grad():
            eps = torch.randn_like(true_beta_var)
            true_beta =  true_beta_mean+eps*torch.exp(0.5*true_beta_var)
            beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate, scale=torch.mul(self.noise_parameter+(standard_error),mask)+1e-6)
            #beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate*mask, scale=scale_estimate)
            recon = beta_hat_sampler_2.sample()

        log_weights = (beta_hat_loss + slab_loss + spike_loss).squeeze(-1)
        #log_weights =  log_weights.reshape(batch_size, num_samples) # Shape of [batch_size, K]
        log_weights =  log_weights.reshape(num_samples, batch_size) # Shape of [K, batch_size]
        with torch.no_grad():
            #log_w_tilde = log_weights - torch.logsumexp(log_weights,dim=-1,keepdim=True) # normalize weights over K-samples
            log_w_tilde = log_weights - torch.logsumexp(log_weights,dim=0,keepdim=True) # normalize weights over K-samples
            w_tilde = log_w_tilde.exp()
            if not self.training:
                posterior_beta.register_hook(lambda grad: w_tilde.reshape(batch_size*num_samples).unsqueeze(-1) * grad.reshape(batch_size*num_samples,-1))
        sum_over_iwae_samples = torch.sum(w_tilde*log_weights,axis=0) # Sum over K-samples
        total_loss = -torch.mean(sum_over_iwae_samples)
    ...
    def iwae_latent_variable_update_with_flow_and_decoder(self, x, mask, standard_error, posterior_beta, posterior_dist, logit_spike, selected, 
        ...
        Returns:
            _type_: _description_
        """
        x = x.unsqueeze(1).repeat(1,self.num_samples,1) # to become [batch, num samples, variant_num]

        # Find pi probabilities
        spike = torch.mul(logit_spike.exp(), mask) + 1e-6
        spike = torch.clamp(spike, 1e-6, 1.0-1e-6)

     
        # Find q(z|x) * mask
        mean_z = true_beta_mean.unsqueeze(1).repeat(1, num_samples, 1)
        var_z =true_beta_var.unsqueeze(1).repeat(1, num_samples, 1)
        scale_z = var_z.mul(0.5).exp_()
        log_Q_e_Fx = torch.distributions.Normal(mean_z, scale_z).log_prob(posterior_beta)*mask
        
        # Find p(z)
        p_z_log_prob, p_z_det = self.latent_prior.log_prob(posterior_beta) #[batch, num_samples, num_Variants], [batch, num_samples]
        p_z_log_prob[:,:,0] = p_z_log_prob[:,:,0] + p_z_det 
        p_z_total_log_prob = p_z_log_prob*mask
        # Find p(z) - q(z|x)
        ...
        mask_prior = torch.sum(mask,axis=-1,keepdim=True)*mask
        mask_prior = torch.nan_to_num(torch.pow(mask_prior, torch.tensor(-1)),nan=1.0,posinf=0.0,neginf=0.0)# create uniform probs except where 0
        mask_prior = torch.log(torch.clamp(mask_prior, min=1e-6, max=0.99)) # so that uniform distribution is on a log scale
        prior_probs = torch.distributions.Uniform(low=mask_prior, high=torch.ones_like(mask_prior)).sample() # pi_probs ~ U(log(1/p),1), where p is number of variants, pi ~ Bern(pi_probs.exp())
        p_sampler = torch.distributions.relaxed_bernoulli.RelaxedBernoulli(temperature=self.temperature, logits=prior_probs)
        log_P_d_Fpi = p_sampler.log_prob(pi_samples)*mask
        ...
        #mean_estimate, log_var_estimate = self.decoder(posterior_beta)
        mean_estimate = self.decoder(posterior_beta)
        #scale_estimate = (log_var_estimate.mul(0.5).exp_()).pow(2)
        #mean_estimate = torch.mul(mean_estimate, mask)
        #beta_hat_sampler = torch.distributions.Normal(loc=mean_estimate, scale=torch.mul(self.noise_parameter+(standard_error),mask)+1e-6).log_prob(x)*mask
        beta_hat_sampler = torch.distributions.Normal(loc=mean_estimate, scale=(standard_error)+1e-6).log_prob(x)*mask # Follows estimated beta ~ N(beta, SE^2)
        beta_hat_loss = torch.sum(beta_hat_sampler, axis=-1,keepdim=True)

        check_beta_hat = beta_hat_loss.detach().squeeze(-1).sum(axis=1).mean(axis=0)
        check_slab_loss = slab_loss.detach().squeeze(-1).sum(axis=1).mean(axis=0)
        check_spike_loss = spike_loss.detach().squeeze(-1).sum(axis=1).mean(axis=0)
        
        with torch.no_grad():
            eps = torch.randn_like(true_beta_var)
            true_beta =  true_beta_mean+eps*torch.exp(0.5*true_beta_var)
            #beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate, scale=scale_estimate)
            #beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate*mask, scale=torch.mul(self.noise_parameter+(standard_error),mask)+1e-6)
            beta_hat_sampler_2 = torch.distributions.Normal(loc=mean_estimate, scale=(standard_error)+1e-6)
            recon = beta_hat_sampler_2.sample()*mask

        log_weights = (beta_hat_loss + self.kl_weight_slab* slab_loss + self.kl_weight_spike*spike_loss).squeeze(-1) # [Batch, k-samples]
        ...
        with torch.no_grad():
            ...
            if not self.training:
                #posterior_beta.register_hook(lambda grad: w_tilde.reshape(batch_size*num_samples).unsqueeze(-1) * grad.reshape(batch_size*num_samples,-1))
                posterior_beta.register_hook(lambda grad: w_tilde * grad)
        sum_over_iwae_samples = torch.sum(w_tilde*log_weights,axis=1) # Sum over K-samples
        total_loss = -torch.mean(sum_over_iwae_samples) # mean over batch


        return total_loss, check_beta_hat.detach(), check_slab_loss.detach(), check_spike_loss.detach(), posterior_beta.detach(), true_beta.detach(), recon.detach(), pi_samples, self.latent_prior
    
    def iwae_latent_variable_update_with_flow_and_decoder_no_mask(self, x, mask, standard_error, posterior_beta, posterior_dist, logit_spike, selected, 
                                                           true_beta_mean, true_beta_var, num_samples, batch_size):
        """Importance weighted elbo
        ...
        # Find p(z)
        p_z_log_prob, p_z_det = self.latent_prior.log_prob(posterior_beta)
        p_z_log_prob = p_z_log_prob + p_z_det
        p_z_total_log_prob = p_z_log_prob
        # Find p(z) - q(z|x)
        ...
        # find p(pi)
        p_sampler = torch.distributions.relaxed_bernoulli.RelaxedBernoulli(temperature=self.temperature, probs=torch.tensor(0.5))
        log_P_d_Fpi = p_sampler.log_prob(pi_samples)

        # what if you use flow prior on 
        #p_pi_log_prob, p_pi_det = self.spike_prior.log_prob(pi_samples)
        #p_pi_log_prob = p_pi_log_prob + p_pi_det
        #p_pi_total_log_prob = p_pi_log_prob


        # find p(pi) - q(pi|x)

        iw_qpi_ppi = log_P_d_Fpi - log_Q_e_Fpi
        #iw_qpi_ppi = p_pi_total_log_prob - log_Q_e_Fpi
        spike_loss = torch.sum(iw_qpi_ppi, axis=-1, keepdim=True)
        ...
        with torch.no_grad():
            ...
            true_beta =  true_beta_mean+eps*torch.exp(0.5*true_beta_var)
            beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate, scale=(standard_error+1e-6))
            #beta_hat_sampler_2 =  torch.distributions.Normal(loc=mean_estimate*mask, scale=scale_estimate)*mask
            recon = beta_hat_sampler_2.sample()
    ...
    def forward(self, data, **kwargs):
        ...
        if self.reinforcement_learning:
            ...
            betas, mask = data
            posterior_beta, posterior_dist, critic_mean, log_critic_var, selectors = self.encoder(betas)
            

            # Calculate q(B|estimated beta)
            #posterior_beta, posterior_dist = self.reparameterize(critic_mean, log_critic_var)

             # Calculate selected variants/features q(pi | stimated beta)
            
            # if not self.training: # for viewing computational graph
            #     standard_error = kwargs.get("standard_error", 0)
            #     num_samples = kwargs.get("num_samples", 0)
            #     beta_batch_shape = kwargs.get("beta_batch_shape", 0)
                
            #     latent_loss, beta_hat_loss, kl_slab, kl_spike, \
            #      posterior_beta, true_beta, mean_estimate, \
            #          spikes, prior_dist = self.iwae_latent_variable_update_with_flow_and_decoder(betas, mask, standard_error, 
            #                                                                                       posterior_beta, posterior_dist, selectors, 
            #                                                                                       0, critic_mean, log_critic_var, num_samples, beta_batch_shape)
    ...
    def model_parameters(self):
        ...
        #model_param_list = {'prior': list(self.latent_prior.parameters()), 'encoder': list(self.encoder.parameters())+list(self.decoder.parameters())+[self.noise_parameter]}
        #model_param_list = {'prior': list(self.latent_prior.a_dist.parameters()), 'encoder': list(self.encoder.parameters())+list(self.decoder.parameters())+[self.noise_parameter]} #if using affine transformation
        #model_param_list = {'prior': list(self.latent_prior.parameters()), 'encoder': list(self.encoder.parameters())+list(self.decoder.parameters())+[self.noise_parameter]}
        model_param_list = {'prior': list(self.latent_prior.parameters()), 'encoder': list(self.encoder.parameters())+list(self.decoder.parameters())}
        #model_param_list = {'encoder': list(self.encoder.parameters())+list(self.decoder.parameters())+[self.noise_parameter]}
        #model_param_list = {'encoder': list(self.encoder.parameters())}
